...
struct MusicTrack: Identifiable {
    ...
    var title: String
    var artist: String
    var albumArtURL: URL? // For network images
    var albumArtAssetName: String? // For local images
        ...
        if let track = viewModel.currentTrack {
            VStack(spacing: 16) {
                HStack(spacing: 12) {
                    ...
                    Group {
                        if let assetName = track.albumArtAssetName {
                            ...
                            // --- DEBUG ---
                                                       print("DEBUG: MusicPlayerView - Attempting to load asset: '\(assetName)'")
                                                       if UIImage(named: assetName) == nil {
                                                           print("DEBUG: MusicPlayerView - WARNING: UIImage(named: \"\(assetName)\") returned nil. Asset not found or not in target.")
                                                       } else {
                                                           print("DEBUG: MusicPlayerView - UIImage(named: \"\(assetName)\") successfully loaded.")
                                                       }
                                                       // --- END DEBUG ---
                ...
                // ... (HStack for Controls - No changes)
                 HStack {
                    Spacer()
                    ControlButton(systemName: viewModel.repeatMode == .one ? "repeat.1" : "repeat", size: 36, color: viewModel.repeatMode == .off ? iconColor : accentColor) {
                    ...
                    Spacer()
                     ControlButton(systemName: viewModel.isFavorited ? "heart.fill" : "heart", size: 36, color: viewModel.isFavorited ? accentColor : iconColor) {
